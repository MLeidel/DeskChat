...
#----------------------------------------------------------------------
    ...
    def api_ollama_cloud(self):
        ...
        client = Client(
            ...
            headers={
                'Authorization': f"Bearer {os.environ.get('OMAKEY')}"
            }
        )
    ...
    def api_deepseek(self):
        ...
        try:
            client = OpenAI(api_key=os.environ.get('DSKKEY'), base_url="https://api.deepseek.com")
            resp  = client.chat.completions.create(
                model = self.MyModel,
